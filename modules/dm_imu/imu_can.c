...
static err_t IMU_SendCommand(uint8_t reg_id, uint8_t access, uint32_t data)
{
  ...
  if (imu.can_handle == NULL) return PTR_NULL;
  return STM32CAN_SendByHandle(imu.can_handle, imu.can_id, payload, sizeof(payload));
}

err_t imu_can_init(IMUCAN_t *self, STM32CAN_t *can, uint8_t can_id, uint8_t mst_id)
{
  if ((self == NULL) || (can == NULL)) return PTR_NULL;
  memset(self, 0, sizeof(*self));
  self->can_ = can; self->can_id = can_id; self->mst_id = mst_id;
  self->data.can_id = can_id; self->data.mst_id = mst_id;
...
{
  if (self == NULL) return PTR_NULL;
  if (self->init_error_ != OK) return self->init_error_;
  return STM32CAN_Start(self->can_);
}
...
void imu_can_update(IMUCAN_t *self, uint32_t timeout_ms)
{
  ...
  BaseType_t result = xTaskNotifyWait(SIGNAL_IMUCAN_RAW_READY, UINT32_MAX, &notify_value,
  ...
  if ((result == pdTRUE) && frame_ready_) {
    ...
    self->data = imu;
    self->online_ = true;
  } else self->online_ = false;
}

void imu_init(uint8_t can_id, uint8_t mst_id, CAN_HandleTypeDef *can_handle)
{ imu.can_id = can_id; imu.mst_id = mst_id; imu.can_handle = can_handle; }
void imu_write_reg(uint8_t reg_id, uint32_t data) { (void)IMU_SendCommand(reg_id, CMD_WRITE, data); }
void imu_read_reg(uint8_t reg_id) { (void)IMU_SendCommand(reg_id, CMD_READ, 0u); }
...
void imu_change_com_port(imu_com_port_e p) { imu_write_reg(CHANGE_COM, (uint32_t)p); }
...
static void IMU_UpdateAccel(const uint8_t *p) { for (uint8_t i=0;i<3u;++i) imu.accel[i]=uint_to_float(U16LE(&p[2u+i*2u]),ACCEL_CAN_MIN,ACCEL_CAN_MAX,16u); }
static void IMU_UpdateGyro(const uint8_t *p) { for (uint8_t i=0;i<3u;++i) imu.gyro[i]=uint_to_float(U16LE(&p[2u+i*2u]),GYRO_CAN_MIN,GYRO_CAN_MAX,16u); }
static void IMU_UpdateEuler(const uint8_t *p) { imu.pitch=uint_to_float(U16LE(&p[2]),PITCH_CAN_MIN,PITCH_CAN_MAX,16u); imu.yaw=uint_to_float(U16LE(&p[4]),YAW_CAN_MIN,YAW_CAN_MAX,16u); imu.roll=uint_to_float(U16LE(&p[6]),ROLL_CAN_MIN,ROLL_CAN_MAX,16u); }
static void IMU_UpdateQuaternion(const uint8_t *p)
{
  uint16_t v[4];
  v[0]=(uint16_t)((p[1]<<6)|((p[2]&0xF8u)>>2));
  v[1]=(uint16_t)(((p[2]&3u)<<12)|(p[3]<<4)|((p[4]&0xF0u)>>4));
  v[2]=(uint16_t)(((p[4]&0x0Fu)<<10)|(p[5]<<2)|((p[6]&0xC0u)>>6));
  v[3]=(uint16_t)(((p[6]&0x3Fu)<<8)|p[7]);
  for (uint8_t i=0;i<4u;++i) imu.q[i]=uint_to_float(v[i],QUATERNION_MIN,QUATERNION_MAX,14u);
}
void IMU_UpdateData(uint8_t *p)
{
  if (p==NULL) return;
  switch (p[0]) { case ACCEL_DATA: IMU_UpdateAccel(p); break; case GYRO_DATA: IMU_UpdateGyro(p); break; case EULER_DATA: IMU_UpdateEuler(p); break; case QUAT_DATA: IMU_UpdateQuaternion(p); break; default: break; }
}
