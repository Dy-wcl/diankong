...
static void STM32_UART_TX_Unlock(uint32_t primask)
{
  __set_PRIMASK(primask);
}

//! 两块缓冲必须等长且不重叠，长度受 DMA 16 位 NDTR 限制。
static bool STM32_UART_TX_ValidBuffers(BSP_UART_RawData_t buf0, BSP_UART_RawData_t buf1)
{
  const uintptr_t addr0 = (uintptr_t)buf0.addr_;
  const uintptr_t addr1 = (uintptr_t)buf1.addr_;
  const uintptr_t distance = (addr0 < addr1) ? (addr1 - addr0) : (addr0 - addr1);
  return (buf0.addr_ != NULL) && (buf1.addr_ != NULL) &&
         (buf0.size_ > 0U) && (buf0.size_ <= UINT16_MAX) &&
         (buf0.size_ == buf1.size_) && (distance >= buf0.size_);
}
                                ...
                                STM32UART_TxCompleteCallback_t callback)
{
  ...
  const BSP_UART_t id = (uart_handle != NULL) ? BSP_UART_get_id(uart_handle->Instance) : BSP_UART_ID_ERROR;
...
static STM32UARTDoubleBufTx_t *STM32_UART_DBM_GetObject(DMA_HandleTypeDef *hdma)
{
                                ...
                                STM32UART_TxRefillCallback_t callback)
{
  ...
  if ((uart_handle == NULL) || (dma_buff_0.addr_ == NULL) || (dma_buff_1.addr_ == NULL))
  {
    self->last_error_ = PTR_NULL;
    return self->last_error_;
  }
...
err_t STM32UARTDoubleBufTx_SetTxDMA(STM32UARTDoubleBufTx_t *self)
{
  ...
  if ((huart == NULL) || (huart->hdmatx == NULL) || (huart->hdmatx->Instance == NULL))
  {
    self->last_error_ = PTR_NULL;
    return self->last_error_;
  }
  if (!BSP_UART_is_valid_id(self->id_) || (stm32_uart_double_buf_tx_map[self->id_] != self))
  {
...
err_t STM32UARTDoubleBufTx_Flush(STM32UARTDoubleBufTx_t *self)
{
  ...
  else
  {
    ...
    const HAL_StatusTypeDef status = HAL_DMAEx_MultiBufferStart_IT(
        ...
        (uint32_t)(uintptr_t)self->dma_buff_1_.addr_, (uint32_t)self->dma_buff_0_.size_);
    if (status == HAL_OK)
    {
...
err_t STM32UARTDoubleBufTx_Stop(STM32UARTDoubleBufTx_t *self)
{
  ...
  CLEAR_BIT(huart->Instance->CR3, USART_CR3_DMAT);
  __HAL_DMA_DISABLE_IT(huart->hdmatx, DMA_IT_TC | DMA_IT_HT | DMA_IT_TE | DMA_IT_DME);
