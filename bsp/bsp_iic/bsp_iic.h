...
struct STM32IIC {
  ...
  STM32IIC_Callback_t
      callbacks_[BSP_IIC_CB_NUMBER];  //! 各 HAL I2C 事件对应的用户回调
...
//! 注册 IIC 指定事件类型的用户回调。
err_t STM32IIC_RegisterCallback(STM32IIC_t* self, BSP_IIC_Callback_t type,
                                STM32IIC_Callback_t callback);

//! 更新 IIC 指定事件类型的用户回调，可传入 NULL 清除回调。
void STM32IIC_SetCallback(STM32IIC_t* self, BSP_IIC_Callback_t type,
                          STM32IIC_Callback_t callback);
