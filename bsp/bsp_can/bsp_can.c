...
static err_t BSP_CAN_start_handle(CAN_HandleTypeDef* can_handle) {
  ...
  return (HAL_CAN_ActivateNotification(
              can_handle, CAN_IT_RX_FIFO0_MSG_PENDING |
...
static err_t BSP_CAN_send_std_data(CAN_HandleTypeDef* can_handle,
                                   uint32_t std_id, const uint8_t* data,
                                   size_t size) {
  ...
  return (HAL_CAN_AddTxMessage(can_handle, &tx_header, (uint8_t*)data,
                               &used_mailbox) == HAL_OK)
             ? OK
             : FAILED;
}
...
static void BSP_CAN_RX_ISR_Handler(CAN_HandleTypeDef* can_handle,
  ...
  if (HAL_CAN_GetRxMessage(can_handle, fifo, &rx_header, frame.data_) !=
      HAL_OK) {
  ...
  frame.id_ = (rx_header.IDE == CAN_ID_STD) ? rx_header.StdId : rx_header.ExtId;
  ...
  if (frame.size_ < BSP_CAN_DATA_SIZE) {
    ...
    memset(&frame.data_[frame.size_], 0, BSP_CAN_DATA_SIZE - frame.size_);
  }
